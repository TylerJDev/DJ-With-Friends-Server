                            ...
                            _request2.default.get(playbackOptions, function (error, response, body) {
                                ...
                                setTimeout(function () {
                                    ...
                                    if (premium_hosts.length) {
                                        ...
                                        if (_index.globalStore.rooms[currentRoom] !== undefined && _index.globalStore.rooms[currentRoom].queue[0] !== undefined) {
                                            _index.globalStore.rooms[currentRoom].noHost = true;
                                            _index.globalStore.rooms[currentRoom].playData = {
                                                'queueCurrent': _index.globalStore.rooms[currentRoom].queue[0],
                                                'currentUser': currentUser,
...
var validateAlbumImage = function validateAlbumImage(src) {
    // Array is expected from passed value {src}
    if (!src.length) {
        return '';
    }

    var newSrc = src.filter(function (current) {
